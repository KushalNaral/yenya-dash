...
const onSubmit = form.handleSubmit(async (values) => {
  ...
  try {
    console.log("Submitting reset password form with:", values);
    ...
    console.log("Reset password error caught:", e);
  ...
  >
    ...
    >
      <div class="text-center">
        ...
        <p class="mt-2 text-sm text-muted-foreground">Reset your password</p>
      </div>
      <form :validation-schema="schema" @submit="onSubmit">
        <div class="space-y-6">
          ...
          <FormField v-slot="{ componentField }" name="token" type="hidden">
            <FormControl>
          ...
          <FormField v-slot="{ componentField, errorMessage }" name="password">
            <FormItem>
              <FormLabel class="text-foreground">New Password</FormLabel>
              <FormControl>
                <div class="relative">
                  <LucideLock
                    class="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-muted-foreground"
      ...
      <p v-if="success" class="mt-2 text-center text-sm text-green-600">
        {{ success }}
      </p>
